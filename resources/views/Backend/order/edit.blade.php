    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col">
                    <h3 class="page-title">Update Order <i class="fa fa-coffee "></i></h3>
                    <ul class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboard</a></li>
                        <li class="breadcrumb-item"><a href="{{ route('order.take') }}">Update Order</a></li>
                    ...
                    <div class="card-body">
                        ...
                        <form method="POST" action="{{ route('order.update',$orders->id) }}">
                            ...
                            <div class="input-fields">
                                            ...
                                            <select class="form-control" name="table_id" id="table_id">
                                                ...
                                                @foreach ($tables as $table)
                                                    <option value="{{ $table->id }}"
                                                        {{ $orders->table_id == $table->id ? 'selected' : '' }}>
                                                        {{ $table->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                ...
                                @foreach ($orders->orderItems as $abc=> $orderitem)
                                <input type="hidden" name="" id="CountOrderItem" value="{{count($orders->orderItems)}}">
                                    <div id="RemoveDeleted-{{ $orderitem->id }}">

                                        <input type="hidden" name="inputs[{{$abc}}][orderitem_id]" value="{{$orderitem->id}}">
                                        <div class="row">
                                            <div class="col-12 col-sm-6 col-md-4">
                                                <div class="form-group local-forms">
                                                    <label>Items <span class="login-danger">*</span></label>
                                                    <select class="form-control myDropdown" name="inputs[{{$abc}}][menu_item_id]"
                                                        id="menu_item">
                                                        ...
                                                        @if ($menuitem)
                                                            @foreach ($menuitem as $key => $value)
                                                                <option value="{{ $value->id }}"
                                                                    ...
                                                                    @if ($orderitem->menu_item_id == $value->id) selected @endif>
                                                                    {{ $value->name }}</option>
                                                            @endforeach
                                                        @endif
                                                    </select>
                                            ...
                                            <div class="col-12 col-sm-6 col-md-3">
                                                <div class="form-group local-forms">
                                                    ...
                                                    <input type="number"
                                                        ...
                                                        name="inputs[{{$abc}}][quantity]" id="quantity"
                                                        value="{{ $orderitem->quantity }}" placeholder="Item quantity">
                                                    @error('quantity')
                                                        <p style="color: indianred;margin-top: 5px">{{ $message }}</p>
                                                    @enderror
                                                </div>
                                            ...
                                            <div class="col-12 col-sm-6 col-md-3">
                                                <div class="form-group local-forms">
                                                    ...
                                                    <input type="number"
                                                        ...
                                                        name="inputs[{{$abc}}][price]" id="price"
                                                        value="{{ $orderitem->price }}" placeholder="Item price" readonly>
                                                    @error('price')
                                                        <p style="color: indianred;margin-top: 5px">{{ $message }}</p>
                                                    @enderror
                                                </div>
                                            ...
                                            <div class="col-md-2">
                                                <a class="btn btn-danger mb-4" title="Cancel Order"
                                                    id="CancelOrder-{{ $orderitem->id }}"><i class="fa fa-trash"></i></a>
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
                                ...
                                <div class="col-md-12">
                                    <a class="btn btn-primary mb-4" title="add more" id="addmore">+ Add More
                                        Orders</a>
                                </div>
...
@section('scripts')
    ...
    <script>
       ...
       //alert(xtotal); 
        $(document).ready(function() {
            ...
            var x = xtotal // initialize counter for input fields
    ...
    @foreach ($orders->orderItems as $orderitem)
        <script>
            $('#CancelOrder-{{ $orderitem->id }}').click(function(e) {
                e.preventDefault();
                //alert('ok');
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        $('#RemoveDeleted-{{ $orderitem->id }}').hide();
                        location.reload();
